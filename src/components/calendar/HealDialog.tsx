    ...
    <AnimatePresence>
      {healPrompt && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/15 backdrop-blur-sm"
          onClick={dismissHeal}
        >
          <motion.div
            ...
            className="glass-strong relative w-full max-w-sm rounded-2xl p-6 soft-shadow"
          >
            <button onClick={dismissHeal} className="absolute right-4 top-4 text-muted-foreground hover:text-foreground transition-colors">
              <X className="h-4 w-4" />
            </button>
            ...
            <div className="flex flex-col gap-2">
              <button
                ...
                className="flex items-center gap-3 rounded-2xl border border-border/50 glass px-4 py-3 text-left text-sm transition-colors hover:bg-accent"
              >
